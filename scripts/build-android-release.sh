...
required=(
  FORTAX_GOOGLE_ANDROID_CLIENT_ID
  FORTAX_MICROSOFT_ANDROID_CLIENT_ID
  FORTAX_MICROSOFT_ANDROID_REDIRECT_URI
  FORTAX_ANDROID_VERSION_CODE
  FORTAX_ANDROID_VERSION_NAME
  FORTAX_ANDROID_KEYSTORE
  FORTAX_ANDROID_KEYSTORE_PASSWORD
  FORTAX_ANDROID_KEY_ALIAS
  FORTAX_ANDROID_KEY_PASSWORD
)
...
if [[ ! -f "$FORTAX_ANDROID_KEYSTORE" ]]; then
  printf 'Production keystore not found: %s\n' "$FORTAX_ANDROID_KEYSTORE" >&2
  exit 1
fi
if [[ "$FORTAX_GOOGLE_ANDROID_CLIENT_ID" != *.apps.googleusercontent.com || "$FORTAX_GOOGLE_ANDROID_CLIENT_ID" == test.* ]]; then
  printf 'FORTAX_GOOGLE_ANDROID_CLIENT_ID is not a production Android OAuth client ID.\n' >&2
  exit 1
fi
if [[ ! "$FORTAX_MICROSOFT_ANDROID_CLIENT_ID" =~ ^[0-9A-Fa-f-]{36}$ || "$FORTAX_MICROSOFT_ANDROID_CLIENT_ID" == 00000000-0000-0000-0000-000000000000 ]]; then
  printf 'FORTAX_MICROSOFT_ANDROID_CLIENT_ID is not a production Entra application ID.\n' >&2
  exit 1
fi
if [[ ! "$FORTAX_MICROSOFT_ANDROID_REDIRECT_URI" =~ ^msauth://com\.fortax\.mail/.+ ]]; then
  printf 'Microsoft redirect must be msauth://in.fortax.mail/<signature-hash>.\n' >&2
  exit 1
fi
if [[ ! "$FORTAX_ANDROID_VERSION_CODE" =~ ^[1-9][0-9]*$ ]]; then
  printf 'FORTAX_ANDROID_VERSION_CODE must be a positive Play version code.\n' >&2
  exit 1
fi
if [[ ! "$FORTAX_ANDROID_VERSION_NAME" =~ ^[0-9]+\.[0-9]+\.[0-9]+([+-][0-9A-Za-z.-]+)?$ ]]; then
  printf 'FORTAX_ANDROID_VERSION_NAME must be a release version such as 1.2.3.\n' >&2
  exit 1
fi
...
cp "$target_dir/android/gradle/app/outputs/bundle/release/app-release.aab" \
  "$artifact_dir/fortax-mail.aab"

if ! command -v jarsigner >/dev/null 2>&1 \
  || ! jarsigner -verify "$artifact_dir/fortax-mail.aab" >/dev/null; then
...
actual_redirect="$(ANDROID_HOME="$android_sdk" "$project_dir/scripts/android-signing-identities.sh" \
  "$artifact_dir/fortax-mail.apk" | sed -n 's/^Microsoft redirect URI: //p')"
if [[ "$actual_redirect" != "$FORTAX_MICROSOFT_ANDROID_REDIRECT_URI" ]]; then
  printf 'The Entra redirect does not match the APK signing certificate.\n' >&2
  printf 'Configured: %s\nActual:     %s\n' \
    "$FORTAX_MICROSOFT_ANDROID_REDIRECT_URI" "$actual_redirect" >&2
  exit 1
fi

ANDROID_HOME="$android_sdk" "$project_dir/scripts/verify-android-apk.sh" \
  "$artifact_dir/fortax-mail.apk" production
printf 'Production artifacts:\n  %s\n  %s\n' \
  "$artifact_dir/fortax-mail.apk" "$artifact_dir/fortax-mail.aab"
