...
binary_path="${1:-$project_dir/target/release/fortax-mail}"
...
app_dir="$output_dir/Fortax Mail.app"
archive_path="$output_dir/fortax-mail-macos-$asset_architecture"
...
mkdir -p \
  ...
  "$app_dir/Contents/Resources/Licenses/Fortax Mail" \
...
cp "$binary_path" "$app_dir/Contents/MacOS/fortax-mail"
...
icon_source="$project_dir/resources/app-icon/fortax-mail-masked.png"
iconset_dir="$output_dir/FortaxMail.iconset"
rm -rf "$iconset_dir"
mkdir -p "$iconset_dir"
...
iconutil -c icns "$iconset_dir" -o "$app_dir/Contents/Resources/FortaxMail.icns"
rm -rf "$iconset_dir"

cp "$project_dir/LICENSE" "$app_dir/Contents/Resources/Licenses/Fortax Mail/LICENSE"
cp -R "$project_dir/LICENSES" "$app_dir/Contents/Resources/Licenses/Fortax Mail/LICENSES"
cp "$project_dir/THIRD_PARTY_NOTICES.md" \
  "$app_dir/Contents/Resources/Licenses/Fortax Mail/THIRD_PARTY_NOTICES.md"
...
python3 "$project_dir/scripts/test-pdf-preview.py" "$app_dir/Contents/MacOS/fortax-mail"
...
rm -rf "$staging"
mkdir -p "$staging"
ditto "$app_dir" "$staging/Fortax Mail.app"
ln -s /Applications "$staging/Applications"
hdiutil create -volname "Fortax Mail" -srcfolder "$staging" -ov -format UDZO \
  "$archive_path.dmg"
