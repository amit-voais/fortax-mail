...
app_dir="$build_dir/fortax-mail.AppDir"
output_path="$build_dir/fortax-mail.AppImage"
staged_output="$build_dir/.fortax-mail.AppImage.new"
...
# FORTAX_APP_FEATURES can enable additional application features.
...
if ldd "$project_dir/target/release/fortax-mail" | grep -q 'libQt'; then
...
mkdir -p \
  ...
  "$app_dir/usr/share/doc/fortax-mail/google-sans-flex" \
  "$app_dir/usr/share/icons/hicolor/512x512/apps" \
  "$app_dir/usr/share/icons/hicolor/scalable/apps" \
  "$app_dir/usr/share/metainfo"
cp "$project_dir/target/release/fortax-mail" "$app_dir/usr/bin/fortax-mail"
python3 "$project_dir/scripts/stage-pdfium.py" linux-x64 "$app_dir/usr/lib/fortax-mail"
app_version="$(python3 "$project_dir/scripts/stage-linux-metadata.py" "$app_dir")"
cp "$project_dir/resources/app-icon/fortax-mail-masked-512.png" "$app_dir/usr/share/icons/hicolor/512x512/apps/in.fortax.mail.png"
cp "$project_dir/resources/app-icon/fortax-mail-masked.svg" "$app_dir/usr/share/icons/hicolor/scalable/apps/in.fortax.mail.svg"
cp "$project_dir/LICENSE" "$app_dir/usr/share/doc/fortax-mail/LICENSE"
cp -R "$project_dir/LICENSES" "$app_dir/usr/share/doc/fortax-mail/LICENSES"
cp "$project_dir/THIRD_PARTY_NOTICES.md" "$app_dir/usr/share/doc/fortax-mail/THIRD_PARTY_NOTICES.md"
cp "$project_dir/resources/fonts/google-sans-flex/OFL.txt" \
  "$app_dir/usr/share/doc/fortax-mail/google-sans-flex/OFL.txt"
cp "$project_dir/resources/fonts/google-sans-flex/README.md" \
  "$app_dir/usr/share/doc/fortax-mail/google-sans-flex/README.md"
cp "$project_dir/resources/AppRun" "$app_dir/AppRun"
chmod +x "$app_dir/AppRun" "$app_dir/usr/bin/fortax-mail"
...
VERSION="$app_version" APPIMAGE_EXTRACT_AND_RUN="${APPIMAGE_EXTRACT_AND_RUN:-1}" \
  "$linuxdeploy" \
  --appdir "$app_dir" \
  --executable "$app_dir/usr/bin/fortax-mail"
...
python3 "$project_dir/scripts/test-pdf-preview.py" "$app_dir/usr/bin/fortax-mail"
...
cp "$project_dir/resources/app-icon/fortax-mail-masked-512.png" "$app_dir/fortax-mail.png"
ln -sfn fortax-mail.png "$app_dir/.DirIcon"
