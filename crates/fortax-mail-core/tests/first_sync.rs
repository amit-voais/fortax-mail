...
//! live. Gated: FORTAX_MAIL_TEST_IMAP=1 (see dovecot_e2e.rs for the container).

use fortax_mail_core::Core;
use fortax_mail_core::config::Paths;
use fortax_mail_core::db::Db;
use fortax_mail_core::events::CoreEvent;
...
async fn first_sync_drains_bodies_fast_and_updates_live() {
    ...
    let creds_file = std::env::temp_dir().join(format!(
        "fortax-mail-firstsync-creds-{}.json",
        std::process::id()
    ));
