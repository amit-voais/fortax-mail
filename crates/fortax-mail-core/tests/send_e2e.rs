...
//! Gated: FORTAX_MAIL_TEST_IMAP=1 and FORTAX_MAIL_TEST_SINK_DIR=<sink output dir>.
...
async fn reply_send_reaches_smtp_and_sent_folder() {
    if std::env::var("FORTAX_MAIL_TEST_IMAP").as_deref() != Ok("1") {
        eprintln!("skipping (set FORTAX_MAIL_TEST_IMAP=1)");
        return;
    }
    let Ok(sink_dir) = std::env::var("FORTAX_MAIL_TEST_SINK_DIR") else {
        eprintln!("skipping (set FORTAX_MAIL_TEST_SINK_DIR)");
        return;
    };
