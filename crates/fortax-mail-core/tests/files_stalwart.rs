...
use fortax_mail_core::files::{dav::DavClient, jmap::FileClient};
use serde_json::json;
...
async fn stalwart_file_round_trip() {
    let base = std::env::var("FORTAX_FILES_TEST_URL").unwrap();
    assert!(
        base.starts_with("http://127.0.0.1:"),
        "Only a disposable loopback server is allowed"
    );
    let user = std::env::var("FORTAX_FILES_TEST_USER").unwrap();
    let secret = std::env::var("FORTAX_FILES_TEST_PASSWORD").unwrap();
    ...
    let parent = root
        ...
        .map(|n| n.id.clone());
    let folder = format!("fortax-test-{}", chrono::Utc::now().timestamp_millis());
    let created = jmap
    ...
    std::fs::write(&source, b"Hello from Fortax").unwrap();
    ...
    assert_eq!(jmap.download(&node).await.unwrap(), b"Hello from Fortax");
    let downloaded = temp.path().join("jmap-download.txt");
    jmap.download_to(&node, &downloaded).await.unwrap();
    assert_eq!(std::fs::read(&downloaded).unwrap(), b"Hello from Fortax");
    ...
    let dav_root = std::env::var("FORTAX_FILES_TEST_DAV").unwrap();
    ...
    let file = listing
        ...
        .unwrap()
        .clone();
    assert_eq!(dav.download(&file).await.unwrap(), b"Hello from Fortax");
    ...
    let file = shared
        ...
        .unwrap();
    assert_eq!(bob_dav.download(file).await.unwrap(), b"Hello from Fortax");
    ...
    dav.relocate_with_policy(
        ...
        "replace-target.txt",
        true,
        fortax_mail_core::files::CollisionPolicy::Replace,
    )
    .await
    .unwrap();
    let files = dav.list(&dir.id).await.unwrap().nodes;
    ...
    jmap.collision = fortax_mail_core::files::CollisionPolicy::Newest;
    assert!(
    ...
    jmap.collision = fortax_mail_core::files::CollisionPolicy::Reject;
    jmap.list(None, "", 0).await.unwrap();
...
impl fortax_mail_core::accounts::credentials::CredentialStore for TestSecrets {
    fn store(
        &self,
        _: i64,
        _: fortax_mail_core::accounts::credentials::Slot,
        _: &str,
    ) -> fortax_mail_core::error::Result<()> {
        Ok(())
    }
    fn load(
        &self,
        _: i64,
        _: fortax_mail_core::accounts::credentials::Slot,
    ) -> fortax_mail_core::error::Result<String> {
        Ok(self.0.clone())
    }
    fn delete(
        &self,
    ...
    fn delete_all(&self, _: i64) -> fortax_mail_core::error::Result<()> {
        Ok(())
    }
}
#[tokio::test]
...
async fn stalwart_durable_sync_and_offline_queue() {
    ...
    let dav = std::env::var("FORTAX_FILES_TEST_DAV").unwrap();
    core.save_file_connection_settings(
        1,
        ConnectionSettings {
            endpoint: dav,
...
async fn check_server_attachment_search(
    ...
    let subject = format!("fortaxsearch{}", chrono::Utc::now().timestamp_millis());
    let mime = format!(
