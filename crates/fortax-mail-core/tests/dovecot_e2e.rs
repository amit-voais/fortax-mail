...
async fn seed_mailbox() {
    ...
    let fixtures = [
        ...
        msg(
            ...
            "[fortax-mail] CI failed on master",
            "Run #4512 failed on master.\r\n",
            None,
            3,
            "List-Id: <fortax-mail.github.com>\r\n",
        ),
        msg(
            5,
...
async fn full_sync_triage_and_search() {
    if !gated() {
        eprintln!("skipping (set FORTAX_MAIL_TEST_IMAP=1 to run)");
        return;
    }
    ...
    let creds_file = std::env::temp_dir().join(format!(
        "fortax-mail-test-creds-{}.json",
        std::process::id()
    ));
    unsafe { std::env::set_var("FORTAX_MAIL_CREDENTIALS_INSECURE_FILE", &creds_file) };
    let _ = tracing_subscriber::fmt()
        .with_env_filter(
            tracing_subscriber::EnvFilter::try_from_default_env()
                .unwrap_or_else(|_| "fortax_mail_core=debug".into()),
        )
        .try_init();
    seed_mailbox().await;
    ...
    wait_for("remote archive replay", Duration::from_secs(90), || async {
        ...
        let creds = fortax_mail_core::imap::ImapCredentials::Password {
            user: "testuser".into(),
            password: "pass".into(),
        };
    ...
    let mut ev_rx = core.bus.subscribe();
    {
        ...
        fortax_mail_core::imap::append(&mut s, "INBOX", raw.as_bytes(), false)
            .await
    ...
    let pushed = tokio::time::timeout(Duration::from_secs(15), async {
        loop {
            match ev_rx.recv().await {
                Ok(fortax_mail_core::events::CoreEvent::MailNew { .. }) => break true,
                Ok(_) => continue,
                Err(_) => break false,
            }
        }
