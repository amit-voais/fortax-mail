...
//! Gated: FORTAX_MAIL_TEST_PERF=1 (seeding takes ~10-30s).

use fortax_mail_core::db::{Db, repo};
use fortax_mail_core::models::{ThreadCursor, View};
use std::time::Instant;

#[tokio::test(flavor = "multi_thread")]
async fn hundred_k_messages_stay_instant() {
    if std::env::var("FORTAX_MAIL_TEST_PERF").as_deref() != Ok("1") {
        eprintln!("skipping (set FORTAX_MAIL_TEST_PERF=1 to run)");
        return;
    }
    ...
    let hits = db
        .read(|conn| {
            let q = fortax_mail_core::search::parse("quarterly numbers");
    ...
    let chronological = db
        .read(|conn| {
            let q = fortax_mail_core::search::parse("update");
    ...
    let hits2 = db
        .read(|conn| {
            let q = fortax_mail_core::search::parse("from:sender42 meeting");
            repo::search::search(conn, &q, 50)
        })
        .await
        .unwrap();
