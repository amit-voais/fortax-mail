...
use fortax_mail_core::{imap, smtp};
    ...
    S: tokio::io::AsyncRead + tokio::io::AsyncWrite + Unpin,
{
    ...
    assert!(command.contains(r#""name" "Fortax Mail""#));
    assert!(command.contains(&format!(r#""version" "{}""#, env!("CARGO_PKG_VERSION"))));
    assert!(command.contains(r#""vendor" "Fortax""#));
    let tag = command.split_whitespace().next().unwrap();
    stream
...
async fn run_imap_move(fixture: ImapMoveFixture) -> fortax_mail_core::error::Result<()> {
...
async fn backup_import_preserves_transport_settings_and_rejects_private_keys() {
    use fortax_mail_core::{Core, config::Paths};
    let temp = tempfile::tempdir().unwrap();
