...
/// into the small deterministic action vocabulary supported by Fortax Mail.
...
mod tests {
    ...
    fn rule_wins_over_heuristic() {
        ...
        let news = label_id(&c, "FortaxMailAutoNews");
        let rule = SplitRule {
            id: 1,
            name: "shop".into(),
            position: 0,
    ...
    fn apply_tab_keeps_one_auto_label() {
        ...
        let marketing = format!("label:{}", label_id(&c, "FortaxMailAutoMarketing"));
        let news = format!("label:{}", label_id(&c, "FortaxMailAutoNews"));
        ...
        assert_eq!(labels, vec![label_id(&c, "FortaxMailAutoNews")]);
    ...
    fn routed_thread_leaves_the_other_tab() {
        ...
        let t = seed_auto_thread(&c, "offers@shop.example", "50% off everything", true);
        route_thread_deterministic(&c, &[], false, t).unwrap();
        let marketing = label_id(&c, "FortaxMailAutoMarketing");
