...
pub trait CredentialStore: Send + Sync {
    ...
    fn check_available(&self) -> Result<()> {
        ...
        let secret = "fortax-credential-probe-v1";
...
#[cfg(not(any(target_os = "android", target_os = "ios")))]
const CHUNK_MARKER: &str = "\u{0}fortax-mail-chunks:";
#[cfg(not(any(target_os = "android", target_os = "ios")))]
const GENERATION_CHUNK_MARKER: &str = "\u{0}fortax-mail-chunks:v2:";
...
/// FORTAX_MAIL_CREDENTIALS_INSECURE_FILE=<path> stores secrets as plaintext
/// JSON.
...
fn insecure_file() -> Option<std::path::PathBuf> {
    std::env::var("FORTAX_MAIL_CREDENTIALS_INSECURE_FILE")
        .ok()
        .filter(|p| !p.is_empty())
