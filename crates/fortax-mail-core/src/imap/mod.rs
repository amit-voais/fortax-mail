...
/// Dev/self-hosted escape hatch: FORTAX_MAIL_TLS_INSECURE=1 disables certificate
/// verification (e.g. self-signed Dovecot). Off by default; never set it for
/// real accounts.
pub fn tls_insecure() -> bool {
    std::env::var("FORTAX_MAIL_TLS_INSECURE")
        .map(|v| v == "1")
        .unwrap_or(false)
}
...
pub fn tls_connector() -> TlsConnector {
    static CONFIG: once_cell::sync::Lazy<Arc<rustls::ClientConfig>> =
        once_cell::sync::Lazy::new(|| {
            ...
            if tls_insecure() {
                tracing::warn!(
                    "FORTAX_MAIL_TLS_INSECURE=1: TLS certificate verification is DISABLED"
                );
...
/// Identify Fortax Mail to servers that implement RFC 2971. Some services
