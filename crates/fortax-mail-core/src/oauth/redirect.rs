...
mod tests {
    ...
    fn redirect_parser_decodes_code_and_state_and_rejects_denial() {
        assert_eq!(
            parse_redirect_uri("in.fortax.mail.oauth://callback?code=a%2Fb&state=ok").unwrap(),
            AuthCode {
                code: "a/b".into(),
                state: Some("ok".into())
            }
        ...
        assert!(
            parse_redirect_uri("in.fortax.mail.oauth://callback?error=access_denied&state=ok")
                .is_err()
        );
    }
    ...
    fn redirect_guard_rejects_wrong_state_expiry_and_duplicate_delivery() {
        ...
        assert!(
            guard
                .accept("in.fortax.mail.oauth://callback?code=nope&state=wrong")
                .unwrap_err()
                .to_string()
        ...
        assert_eq!(
            guard
                .accept("in.fortax.mail.oauth://callback?code=usable&state=right")
                .unwrap()
                .code,
            "usable"
        );
        assert!(
            guard
                .accept("in.fortax.mail.oauth://callback?code=again&state=right")
                .unwrap_err()
                .to_string()
        ...
        assert!(
            expired
                .accept("in.fortax.mail.oauth://callback?code=late&state=right")
                .unwrap_err()
                .to_string()
                .contains("expired")
        );
