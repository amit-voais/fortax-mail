...
impl Paths {
    ...
    pub fn default_dirs() -> Result<Self> {
        ...
        Ok(Self::new(
            data_root.join("fortax-mail"),
            cache_root.join("fortax-mail"),
        ))
    }
    ...
    pub fn default_dirs() -> Result<Self> {
        ...
        Ok(Self::new(
            root.join("Library/Application Support/Fortax"),
            root.join("Library/Caches/Fortax"),
        ))
    }
    ...
    pub fn db_file(&self) -> PathBuf {
        self.data_dir.join("fortax-mail.db")
    }
    ...
    pub fn calendar_db_file(&self) -> PathBuf {
        self.data_dir.join("fortax-calendar.db")
    }

    pub fn files_db_file(&self) -> PathBuf {
        self.data_dir.join("fortax-files.db")
    }
...
mod tests {
    ...
    fn platform_paths_separate_durable_and_reconstructable_state() {
        ...
        assert_eq!(paths.db_file(), root.join("data/fortax-mail.db"));
        assert_eq!(
            paths.calendar_db_file(),
            root.join("data/fortax-calendar.db")
        );
        assert_eq!(
