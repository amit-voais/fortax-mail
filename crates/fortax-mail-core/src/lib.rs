...
impl Core {
    ...
    pub async fn create_event(&self, args: CreateEventArgs) -> Result<CalendarEvent> {
        ...
        let uid = format!("{}-{}@fortax-mail", now_ms(), crate::mime::rand_token());
...
/// `FORTAX_MAIL_RESOURCE_DIR` to the platform-specific resource directory.
#[cfg(feature = "local-embeddings")]
fn bundled_model_dir(key: &str) -> Option<std::path::PathBuf> {
    let base = std::env::var_os("FORTAX_MAIL_RESOURCE_DIR")?;
